  ...
  <div class="min-h-screen w-full p-8 bg-gray-50 flex justify-center">
    <div class="w-full max-w-5xl bg-white rounded shadow p-6">
      ...
      <table class="w-full border-collapse border border-gray-300 text-sm">
        ...
        <tbody>
          <tr v-for="(invoice, index) in invoices" :key="invoice.number" class="hover:bg-gray-50">
          ...
          <tr v-if="invoices.length === 0">
            <td colspan="5" class="p-4 text-center text-gray-500 italic">No saved invoices found.</td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded shadow-lg max-w-4xl w-full p-6 overflow-auto max-h-[80vh]">
          ...
          <h2 class="text-2xl font-bold text-[#103355] mb-4">Invoice Details - {{ selectedInvoice.number }}</h2>
          ...
          <table class="w-full border-collapse border border-gray-300 text-sm mt-6">
            ...
            <tbody>
              <tr v-for="(item, i) in selectedInvoice.items" :key="i" class="border-t">
...
function downloadInvoice(index) {
  ...
  doc.text(`Total: ZMW ${invoice.total.toFixed(2)}`, 150, doc.autoTable.previous.finalY + 10)
...
function printInvoice(index) {
  ...
  const html = `
    ...
    <body>
      ...
      <p style="text-align:right; font-weight:bold; margin-top:20px;">Total: ZMW ${invoice.total.toFixed(2)}</p>
    </body>
    </html>
  `
